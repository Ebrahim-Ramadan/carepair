...
export function TicketForm({ onSuccess }: TicketFormProps) {
  ...
  const toggleRepairPart = (part: string) => {
    setRepairParts((prev) => (prev.includes(part) ? prev.filter((p) => p !== part) : [...prev, part]))
  }
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {REPAIR_PARTS.map((part) => (
            <div key={part} className="flex items-center space-x-2">
              <Checkbox id={part} checked={repairParts.includes(part)} onCheckedChange={() => toggleRepairPart(part)} />
              <Label htmlFor={part} className="cursor-pointer text-sm font-normal">
                {part}
              </Label>
            </div>
          ))}
